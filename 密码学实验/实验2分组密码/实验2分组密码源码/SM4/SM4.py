...
def main():
    # 密钥输入（16字节的十六进制字符串）
    key_str = input("请输入密钥 (32位16进制字符): ").strip()
    if len(key_str) != 32 or not all(c in "0123456789abcdefABCDEF" for c in key_str):
        raise ValueError("密钥格式错误")
    mk = bytes.fromhex(key_str)

    # 明文输入（16字节的十六进制字符串）
    plaintext_hex = input("请输入明文 (32位16进制字符): ").strip()
    if len(plaintext_hex) != 32 or not all(c in "0123456789abcdefABCDEF" for c in plaintext_hex):
        raise ValueError("明文格式错误")
    plaintext = bytes.fromhex(plaintext_hex)

    # 加密
    ciphertext = sm4_encrypt_without_padding(mk, plaintext)
