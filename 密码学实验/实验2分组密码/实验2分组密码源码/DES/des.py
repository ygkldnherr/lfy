...
class DES(object):
    ...
    def Decrypt(self, e: bytes) -> bytes:
        ...
        for i in range(len(blocks) // 2):
            ...
            cblocks.append(high)
            cblocks.append(low)
            decrypted = blocksTobytes(cblocks)
...
def hexstr_to_bytes(hex_str):
    """将十六进制字符串转换为字节序列"""
    return bytes.fromhex(hex_str.decode() if isinstance(hex_str, bytes) else hex_str)


if __name__ == '__main__':
    # 测试1：十六进制输入（原始DES测试）
    plain_hex = "4E6F772069732074"
    key_hex = "0123456789ABCDEF"

    m = bytes.fromhex(plain_hex)  # 直接转换十六进制
    ...
    # 测试2：字符串输入（新增功能）
    plain_text = "少年智则国智，少年富则国富，少年强则国强，少年独立则国独立，少年自由则国自由，少年进步则国进步，少年胜于欧洲则国胜于欧洲，少年雄于地球则国雄于地球。红日初升，其道大光。河出伏流，一泻汪洋。潜龙腾渊，鳞爪飞扬。乳虎啸谷，百兽震惶。鹰隼试翼，风尘翕张。奇花初胎，矞矞皇皇。干将发硎，有作其芒。天戴其苍，地履其黄。纵有千古，横有八荒。前途似海，来日方长。美哉我少年中国，与天不老！壮哉我中国少年，与国无疆！"
    key_str = "0123456789ABCDEF"

    m = plain_text.encode('utf-8')  # 直接编码字符串
