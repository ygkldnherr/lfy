...
class AES:
    ...
    def __init__(self, key):
        self.key = key
        self.round_keys = self.round_key_generator(self._16bytes2num(key))
    ...
    def cfb_encrypt(self, plaintext: bytes, iv: bytes, segment_size=128) -> bytes:
        """
        CFB模式加密
        :param plaintext: 明文字节流
        :param iv: 16字节初始化向量
        :param segment_size: 分段位数（支持128/64/32/16/8）
        :return: 密文字节流
        """
        if len(iv) != 16:
            raise ValueError("IV必须为16字节")
        if segment_size not in (128, 64, 32, 16, 8):
            raise ValueError("不支持的segment_size")

        block_size = 16  # AES块大小固定16字节
        shift_register = bytearray(iv)  # 移位寄存器
        ciphertext = bytearray()

        # 计算分段参数
        s = segment_size // 8  # 分段字节数
        total_blocks = ceil(len(plaintext) / s)

        for i in range(total_blocks):
            # 生成密钥流
            aes_block = bytes(shift_register)
            encrypted_block = self.aes_encrypt(aes_block, self.round_keys)

            # 截取密钥流段
            key_stream = encrypted_block[:s]

            # 处理当前明文段
            plaintext_block = plaintext[i * s: (i + 1) * s]
            ciphertext_block = bytes([p ^ k for p, k in zip(plaintext_block, key_stream)])
            ciphertext.extend(ciphertext_block)

            # 更新移位寄存器
            shift_register = shift_register[s:] + ciphertext_block

        return bytes(ciphertext)

    def cfb_decrypt(self, ciphertext: bytes, iv: bytes, segment_size=128) -> bytes:
        """
        CFB模式解密
        :param ciphertext: 密文字节流
        :param iv: 16字节初始化向量
        :param segment_size: 分段位数（必须与加密时相同）
        :return: 明文字节流
        """
        if len(iv) != 16:
            raise ValueError("IV必须为16字节")
        ...
        s = segment_size // 8
        total_blocks = ceil(len(ciphertext) / s)
        ...
        for i in range(total_blocks):
            ...
            # 处理当前密文段
            ciphertext_block = ciphertext[i * s: (i + 1) * s]
            plaintext_block = bytes([c ^ k for c, k in zip(ciphertext_block, key_stream)])
...
if __name__ == '__main__':
    # 密钥处理（支持任意长度，自动补齐/截断到16字节）
    key_str = "0123456789abcdeffedcba9876543210"
    key = key_str.encode('utf-8').ljust(16, b'\0')[:16]
    aes = AES(key)

    # 原始明文（包含中文和特殊字符）
    plaintext = "少年智则国智，少年富则国富，少年强则国强，少年独立则国独立，少年自由则国自由，少年进步则国进步，少年胜于欧洲则国胜于欧洲，少年雄于地球则国雄于地球。红日初升，其道大光。河出伏流，一泻汪洋。潜龙腾渊，鳞爪飞扬。乳虎啸谷，百兽震惶。鹰隼试翼，风尘翕张。奇花初胎，矞矞皇皇。干将发硎，有作其芒。天戴其苍，地履其黄。纵有千古，横有八荒。前途似海，来日方长。美哉我少年中国，与天不老！壮哉我中国少年，与国无疆！"
    plaintext_bytes = plaintext.encode('utf-8')

    # 加密参数
    iv = AES.generate_iv()
    segment_size = 128  # 可修改为64/32/16/8

    # CFB加密
    ciphertext = aes.cfb_encrypt(plaintext_bytes, iv, segment_size)
    print(f"加密结果 (hex): {ciphertext.hex()}")
    print(f"IV (hex): {iv.hex()}")

    # CFB解密
    decrypted_bytes = aes.cfb_decrypt(ciphertext, iv, segment_size)
